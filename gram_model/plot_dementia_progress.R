...
plot_dementia_progress  <- function(b0, b1, b11, b12, b2, b3, b4, b1edu, b1hisp, b1ob) {
  ...
  #------- Initialize vectors: Make vectors with “n_t+1” elements to trace the number of people by health state category, then zero out the values.
  v_total_pop_alive  <-
    v_total_pop_dead  <-
    v_pop_cog_imp_none <-
    v_pop_cog_imp_mci <-  
  ...
  log_output(3, sprintf("Simulating cycle t=%s/%s", t, n_t), log_file) 
     ...
     # set.seed(as.integer(round(m_rand_unif[i, t])))
      ...
      if (m_alive[i,t] == 1) { 
        ...
        #-- (4) Calculate the mortality for the cognitively intact then the mortality by severity of cognitive impairment using the inputs calculated above. 
        ...
        #=> this line is for version before the age-adjustment for the mortality of the severely demented was made. So commented out.
