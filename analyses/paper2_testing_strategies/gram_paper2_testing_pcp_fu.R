...
## Reporting results ####
u3pcppos_rand50 <- latest_rds("u3pcppos_rand50")$output
s1pcppos_emr <- latest_rds("s1pcppos_emr")$output
r1pcppos <- latest_rds("r1pcppos")$output

subset_age_65 <- as.data.frame(t(u3pcppos_rand50[65-50+1,,])) # rows are IDs, cols are attributes
subset_age_67 <- as.data.frame(t(u3pcppos_rand50[67-50+1,,])) # rows are IDs, cols are attributes
...
scenario_arrays <- list(
  u3pcppos_rand50 = u3pcppos_rand50,
  s1pcppos_emr = s1pcppos_emr,
  r1pcppos = r1pcppos
)
...
prev_in_first_test$u3pcppos_rand50$n_tested / sum(prev_in_undx_67$n)
prev_in_first_test$s1pcppos_emr$n_tested / sum(prev_in_undx_65$n)
prev_in_first_test$r1pcppos$n_tested / sum(prev_in_undx_65$n)

# Calculate total CI prev at first test
sum(prev_in_first_test$u3pcppos_rand50$prev_by_status$prev[c(1,3)])
sum(prev_in_first_test$s1pcppos_emr$prev_by_status$prev[c(1,3)])
sum(prev_in_first_test$r1pcppos$prev_by_status$prev[c(1,3)])
