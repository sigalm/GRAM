...
plot_dementia_progress  <- function(b0, b1, b11, b12, b2, b3, b4, b1edu, b1hisp, b1ob) {
  ...
  set.seed(20240202)                                # set seed using a fixed number to generate random number from a uniform distribution
  ...
  #------- Initialize vectors: 
  #       Make vectors with “n_t+1” elements to trace the number of people by health state category, then zero out the values.
  v_total_pop_alive  <-
    v_total_pop_dead  <-
    v_pop_cog_imp_none <-
    v_pop_cog_imp_mci <-  
  ...
  #----- 1st LOOP: looping through i at cycle t=1: —-
#  this loop is to make the following vectors for cycle “t = 1,” which are needed to calculate “numerator” & “denominator” in Step (3)
#        v_total_pop_alive[t], v_pop_cog_imp_none[t], v_pop_cog_imp_mci[t], v_pop_cog_imp_mild_dementia[t],  
#         v_pop_cog_imp_mod_dementia[t], v_pop_cog_imp_severe_dementia[t]


# Calculate the probability of dying next year for those cognitively intact at cycle t=1.
  t <- 1
    for (i in 1:n_i) {        
      ...
      #- Thus, we adjust the probability of dying for the severity of dementia based on the Relative Risks (RRs) with the 
      #         Cognitively Intact being its reference group. The RRs are from Anderson 2010 https://pubmed.ncbi.nlm.nih.gov/20110702/ 
      #- We take the following steps (1), (2), (3), and (4) to calculate the intended probability. 
      
      if (m_alive[i,t] == 1) { 
        #-- (1) Count the number of people in each cognitive impairment state (none, mci, mild dementia, moderate dementia, severe dementia)
        #   in cycle t. (The current criterion for categorization may change). We need this count to calculate the probability of 
        #   dying next year by disease state. 
    ...
    v_total_pop_alive[t] <- 
      v_pop_cog_imp_none[t] + 
      v_pop_cog_imp_mci[t] + 
      v_pop_cog_imp_mild_dementia[t] + 
      v_pop_cog_imp_mod_dementia[t]+ 
      v_pop_cog_imp_severe_dementia[t]         
    
    v_total_pop_dead[t]  <- n_i - v_total_pop_alive[t]    
...
for (t in 1:(n_t-1)) {                    # start loop for time cycles: “t in 1:n_t” means “from t=1 to t=n_t”
  ...
  log_output(3, sprintf("Simulating cycle t=%s/%s", t, n_t), log_file) 
      ...
      if (m_alive[i,t] == 1) { 
        ...
        # The Relative Risks (RRs) from the Anderson paper are not age-specific. Considering that mortality is generally very low 
        #     at an early age, it is likely that individuals with severe dementia will have a higher mortality than the average 
        #      for that age group. However, at a later age, when mortality is generally higher, the mortality of those with 
        #       severe dementia may not be as high as at an earlier age. 
        ...
        denom <- 
          v_pop_cog_imp_none[t] + 
          v_pop_cog_imp_mci[t] * 1.82 + 
          v_pop_cog_imp_mild_dementia[t] * 2.92 + 
          v_pop_cog_imp_mod_dementia[t] * 3.85 + 
          v_pop_cog_imp_severe_dementia[t] * rr_sev_dement  
        ...
        #-- (4) Calculate the mortality for the cognitively intact then the mortality by severity of cognitive impairment 
        #           using the inputs calculated above. 
        ...
        #- Determine whether this person is alive or dead at “t+1” based on the Bernoulli distribution - a special case of binomial -
        #       using the probability of dying next year calculated above.  
...
##### Now that we have age-adjusted and RR adjusted mortality, we proceed with transition probability.

      #-- Calculate the MCI transition probability for individual i in cycle t
      x <- b0 + 
        b1*(m_age[i,t]-50) + 
        b2*dementia_data$edu_num[i] + 
        b3*dementia_data$hisp_black[i] + 
        b4*dementia_data$obesity_num[i] + 
        b11*(m_age[i,t]-50)^2 + 
        b12*(m_age[i,t] -50)^3 + 
        b1edu * (m_age[i,t] -50) * dementia_data$edu_num[i]  + 
        b1hisp* (m_age[i,t] -50) * dementia_data$hisp_black[i] + 
        b1ob * (m_age[i,t] -50) * dementia_data$obesity_num[i] 
      
      m_p_none_to_mci <- exp(x) / (1 + exp(x))                          # logistic function
      
      #-- Assign next cycle’s flag for being cognitively impaired (MCI or worse) 
      #- If the MCI flag at t is 1, then the MCI flag at t+1, t+2, etc is always 1. (MCI does not get better.) 
      #- if this MCI flag at t is not 1, then assign MCI flag at t+1 based on the Bernoulli distribution - a special case of binomial - 
      #       using the calculated transition probability
     
      # Set the MCI_flag value at “t+1”, depending on the MCI_flag value at t 
      if (m_mci_flag[i, t] == 1) {
        m_mci_flag[i, (t+1):ncol(m_mci_flag)] <- 1          # Set t+1, t+2, etc. to 1, if m_mci_flag[i, t] is 1
      ...
      if (m_mci_flag[i, t] != 1 & m_mci_flag[i, t+1] == 1) {
        ...
        m_mmse[i, t+1] <- 29    
# Check with Jim: When the MCI flag turns on (i.e. at t+1), MMSE at that cycle (t+1) is 29, if we lower/update MMSE 1 year later, (i.e. at t+2).
      } else if (m_mci_flag[i, t] != 1 & m_mci_flag[i, t+1] != 1) {
        m_mmse[i, t+1] <- 30
        m_time_since_mci_onset[i,t+1] <- 0
      }  else if (m_mci_flag[i, t] == 1 & is.na(m_mmse[i, t+1])) {   # then m_mci_flag[i, t+1] == 1 by design
        m_time_since_mci_onset[i,t] <-  m_age[i,t] - dementia_data$age_at_mci_onset[i] + 1
       
      # THIS WILL CHANGE FOR CDR  
         m_delta_mmse[i,t+1] <- -5.4663 - 0.4299 * min(m_mmse[i,t], 9) - 0.0042 * max(0, min(m_mmse[i,t]-9,9)) + 0.1415 * max(0, min(m_mmse[i,t]-18,12)) - 0.0791 * (m_mmse[i,t] - 30) / m_time_since_mci_onset[i,t] + 0.0747 * min(80, dementia_data$age_at_mci_onset[i]) 
        m_mmse[i,t+1] <- round(min(30, max(0, m_delta_mmse[i,t+1] + m_mmse[i,t])))      
       }
      
      #-- If the MCI flag at t+1 equals 1, then update (lower) MMSE at t+2 using the equation for rate of MMSE change (Getsios, 2010)
      #- Note that applying the rate of change equation can yield negative MMSE value, in which case, MMSE is set to 0.
      if (!is.na(m_mci_flag[i,t+2]) ) {         # then (m_mci_flag[i, t+2] == 1   && m_mci_flag[i, t+1] == 1) by design
        m_time_since_mci_onset[i,t+1] <-  m_age[i,t+1] - dementia_data$age_at_mci_onset[i] + 1
        m_delta_mmse[i,t+2] <- -5.4663 - 0.4299 * min(m_mmse[i,t+1], 9) - 0.0042 * max(0, min(m_mmse[i,t+1]-9,9)) + 0.1415 * max(0, min(m_mmse[i,t+1]-18,12)) - 0.0791 * (m_mmse[i,t+1] - 30) / m_time_since_mci_onset[i,t+1] + 0.0747 * min(80, dementia_data$age_at_mci_onset[i]) 
        m_mmse[i,t+2] <- round(min(30, max(0, m_delta_mmse[i,t+2] + m_mmse[i,t+1])))
      } else {                         # then m_mci_flag[i,t+1] != 1 by design
               m_time_since_mci_onset[i,t+1] <- 0  
              } 
...
end_time <- Sys.time()
log_output(100, paste0("Total run time: ", 
